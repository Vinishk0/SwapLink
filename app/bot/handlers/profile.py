...
def links(bot_username: str, user: User) -> tuple[str, str]:
...
async def render_profile(
    ...
    ref_link, share_url = links(bot_username, user)
    await ui.show(
        event,
        state,
        texts.profile(user, summary, settings, ref_link),
        kb.profile(ref_link=ref_link, share_url=share_url),
    )
...
async def cb_profile(
    ...
    await render_profile(query, session, state, user, settings, bot_username)
    await query.answer()
...
async def cb_enter_code(query: CallbackQuery, state: FSMContext, user: User) -> None:
    ...
    await ui.show(query, state, texts.ask_ref_code(), kb.cancel_input("profile"))
    await query.answer()
